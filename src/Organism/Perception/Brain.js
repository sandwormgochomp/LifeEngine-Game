...
class Brain {
    ...
    copy(brain) {
        this.states = JSON.parse(JSON.stringify(brain.states));
    ...
    evaluateCondition(transition) {
        ...
        let val2 = parseFloat(transition.value);
    ...
    decide() {
        var direction_weights = {0: 0, 1: 0, 2: 0, 3: 0};
        var best_dir = -1;
    ...
    load(data) {
        if (data.states) {
            if (data.states.length > 0 && data.states[0].decisions === undefined) {
                // Backwards compatibility for previous 2-state array of decisions
                this.states = data.states.map((decs, i) => ({
                    name: "State " + (i+1),
                    decisions: decs,
                    actions: {},
                    transitions: []
                ...
                if (data.condition !== undefined && data.condition > 0 && this.states.length > 1) {
                    if (data.condition === 1) {
            ...
            for (let cell of CellStates.all) {
                if (data.decisions[cell.name] === undefined) {
                    data.decisions[cell.name] = 0;
                }
            }
            this.states = [{
                name: "State 1",
