...
class ControlPanel {
        ...
        $('body').keydown( (e) => {
            ...
            switch (e.key.toLowerCase()) {
                ...
                case 'f':
                    ...
                    break;
                case 'w':
                    $('#invincible-wall-drop').click();
                    break;
                case 'r':
                    $('#radiation-drop').click();
                    break;
                case 'g':
                    $('#click-kill').click();
                    break;
    ...
    defineHyperparameterControls() {
        ...
        $('#healer-food-cost').change(function() {
            Hyperparams.healerFoodCost = parseInt($('#healer-food-cost').val());
        });
        $('#explosion-radius').change(function() {
            Hyperparams.explosionRadius = parseInt($('#explosion-radius').val());
        });
        $('#wall-durability').change(function() {
            Hyperparams.wallDurability = parseInt($('#wall-durability').val());
        });
    ...
    updateHyperparamUIValues(){
        ...
        $('#healer-food-cost').val(Hyperparams.healerFoodCost);
        $('#explosion-radius').val(Hyperparams.explosionRadius);
        $('#wall-durability').val(Hyperparams.wallDurability);
    ...
    defineModeControls() {
        ...
        $('.edit-mode-btn').click( function() {
            ...
            switch(this.id) {
                ...
                case "wall-drop":
                    ...
                    break;
                case "invincible-wall-drop":
                    self.setMode(Modes.InvincibleWallDrop);
                    break;
                case "radiation-drop":
                    self.setMode(Modes.RadiationDrop);
                    break;
                case "click-kill":
        ...
        $('#clear-radiation').click( function() {
            if (this.engine.env.radiation_map) {
                this.engine.env.radiation_map.clear();
                this.engine.env.renderFull();
            }
        }.bind(this));
    ...
    setMode(mode) {
        ...
        if (mode == Modes.Edit || mode == Modes.Paint) {
            this.editor_controller.setEditorPanel();
        } else {
            $('#cell-selections').css('display', 'none');
