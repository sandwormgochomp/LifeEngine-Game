...
/* Performance regression floors. These are smoke-level budgets, not
   ...
   fresh world 1x ~60 tps; moderate seeded world at 4x ~250 tps rendered AND
   headless (ratio ~1.0 -- it was 0.34 before the sim/render split); dense
   world (700+ organisms) ~120+ tps headless.
...
test.describe('Performance budgets', () => {
  test('1x runs the sim at real time, and only real time', async ({ page }) => {
    test.setTimeout(30000);
    // Fresh world at 1x: the user-facing "the game runs at full speed" floor.
    await setSpeed(page, 1);
    const tps = await measureTps(page);
    // Floor: a healthy build reads ~60. Ceiling: a multiplier bug that runs
    // extra ticks per firing would read ~120+, so overshoot fails too.
    expect(tps, `1x tick rate was ${tps.toFixed(0)}/s`).toBeGreaterThan(50);
    expect(tps, `1x tick rate was ${tps.toFixed(0)}/s`).toBeLessThan(80);
  });
  ...
  test('Rendering stays decoupled from the sim loop', async ({ page }) => {
    ...
    /* Structural invariant, independent of machine speed and world load: the
       ...
       most), so at 4x the frame rate must be a small fraction of the tick
       ...
       A healthy build reads ~0.25 (60 fps vs ~250 tps). */
    ...
    expect(tps).toBeGreaterThan(100); // 4x must actually multiply ticks
    const ratio = fps / tps;
    expect(
      ratio,
      `at 4x: ${fps.toFixed(0)} fps vs ${tps.toFixed(0)} tps (ratio ${ratio.toFixed(2)}) -- fps tracking tps means render re-coupled to the tick`
    ).toBeLessThan(0.5);
  });
